...
class MakeShiftPort extends EventEmitter {
    ...
    send(t, body) {
        if (this.isOpen) {
            try {
                this.serialPort.flush();
                let h = Buffer$1.from([t]);
                let b = Buffer$1.from(body);
                const buf = Buffer$1.concat([h, b]);
...
export { Events, MakeShiftPort, Msg, PacketType, defaultMakeShiftPortOptions, defaultMsgOptions, devices, filterName, loadDevices, logRank, nspct2, nspect, releaseDevice, setColorize, setLogLevel, setPortAuthorityLogLevel, setWarn, stopScan, strfy };
