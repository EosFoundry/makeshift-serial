...
const defaultMakeShiftPortOptions = {
    portPath: '',
    portInfo: {},
    logLevel: 'all',
    id: '',
...
class MakeShiftPort extends node_events.EventEmitter {
    ...
    emitLog = (msg, lv) => {
        this.emit(Events.TERMINAL.LOG[lv], {
            ...
            level: lv,
            message: msg,
            buffer: node_buffer.Buffer.from(msg),
        });
    };
    ...
    constructor(options = defaultMakeShiftPortOptions) {
        ...
        this.on(Events.DEVICE.DISCONNECTED, () => {
            node_timers.clearInterval(_keepAliveTimers[this.portId]);
        });
    ...
    sendByte(t) {
        if (this.isOpen) {
            try {
                ...
                let buf = node_buffer.Buffer.from([t]);
    ...
    send(t, body) {
        if (this.isOpen) {
            try {
                this.serialPort.flush();
                let h = node_buffer.Buffer.from([t]);
                let b = node_buffer.Buffer.from(body);
                const buf = node_buffer.Buffer.concat([h, b]);
    ...
    static parseStateFromBuffer(data) {
        ...
        function bytesToBin(button, bitCounter) {
            if (bitCounter === 0) {
                return;
            }
            if (button % 2) {
    ...
    async open() {
        this.serialPort = await new serialport.SerialPort({
...
function setPortAuthorityLogLevel(lv) {
    msgen.logLevel = lv;
}
function stopScan() { autoscan = false; }
...
exports.Msg = Msg;
exports.defaultMakeShiftPortOptions = defaultMakeShiftPortOptions;
exports.defaultMsgOptions = defaultMsgOptions;
exports.devices = devices;
exports.filterName = filterName;
exports.loadDevices = loadDevices;
exports.logRank = logRank;
exports.nspct2 = nspct2;
exports.nspect = nspect;
exports.releaseDevice = releaseDevice;
exports.setColorize = setColorize;
exports.setLogLevel = setLogLevel;
exports.setPortAuthorityLogLevel = setPortAuthorityLogLevel;
